...
class DataVisualizer:
    ...
    def _extract_values(self, data: List[Dict], jmespath_expr: str) -> Tuple[List[float], List[str]]:
            ...
            for i in display_points:
                if i < len(self.timestamps):
                    ...
                    if timestamp:
                        ...
                        x_pos = max(3, min(width - len(formatted_time) - 3, x_pos - len(formatted_time) // 2))
                        try:
        ...
        if total_points > 0:
            ...
            if len(coords) >= 4:
                ...
                for i in range(1, len(coords) - 2):
                    ...
                    spline_points = self._calculate_spline_points(x0, y0, x1, y1, x2, y2, x3, y3)
