...
def query_docs_tool(doc_type: str, search_term: Optional[str] = None, include_content: bool = True) -> Dict[str, Any]:
    """
...
def search_doc_content_tool(search_text: str, doc_type: str = "all", case_sensitive: bool = False) -> Dict[str, Any]:
    """
...
def get_sfc_config_examples_tool(component_type: Optional[str] = None, name_pattern: Optional[str] = None) -> Dict[str, Any]:
    """
        ...
        for doc in docs.get("documents", []):
            ...
            for i, block in enumerate(code_blocks):
                try:
                    ...
                    if isinstance(json_content, dict) and any(key in json_content for key in 
                                                            ["name", "adapterType", "targetType", "sources", "sources", "targets"]):
                        component_name = json_content.get("name", "unknown")
                        
                        # Apply name pattern filter if provided
                        if name_pattern and not fnmatch.fnmatch(component_name.lower(), name_pattern.lower()):
                            continue
            ...
            if not isinstance(config["sources"], list):
                ...
                for i, source in enumerate(config["sources"]):
                    ...
                    if "name" not in source:
                        errors.append(f"Source at index {i} is missing required 'name' field")
                    
                    # Check for topics
                    if "topics" in source:
                        if not isinstance(source["topics"], list):
                            errors.append(f"'topics' in source '{source.get('name', i)}' must be an array")
                        else:
                            for j, topic in enumerate(source["topics"]):
                                if not isinstance(topic, dict):
                                    errors.append(f"Topic at index {j} in source '{source.get('name', i)}' must be an object")
                                    continue
                                    
                                if "name" not in topic:
                                    errors.append(f"Topic at index {j} in source '{source.get('name', i)}' is missing required 'name' field")
